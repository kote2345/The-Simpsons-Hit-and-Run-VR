...
void main() {
    ...
    vec3 N=normalize(mat3(draw.normalMatrix)*skinnedNormal);
    ...
    for(int i=0;i<8;++i) {
        ...
        vec3 delta=draw.lights[i].position.xyz-draw.lights[i].position.w*viewPosition.xyz;
        ...
        if(ndl>0.0) {
            ...
            float power=draw.materialParams.x!=0.0?
                pow(max(dot(N,H),0.0),draw.materialParams.x):1.0;
